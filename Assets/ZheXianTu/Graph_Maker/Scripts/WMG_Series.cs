...
public class WMG_Series : MonoBehaviour {
	...
	void Update()
	{
		...
		foreach (Transform child in nodeParent.transform) 
		{
			...
			if (txt) 
			{
				txt.text = _pointValues [m].y.ToString ();
			}
	...
	public void UpdateFromDataSource() {
		if (pointValuesDataSource != null) {
			...
			Debug.Log (dataSourceData[0].y+"----Y");
		}
	}
	...
	void createOrDeletePoints(int pointValuesCount) {
		...
		for (int i = 0; i < pointValuesCount; i++) {
			if (points.Count <= i) {
				...
				Text txt = curObj.GetComponentInChildren<Text> ();
				if (txt) {
					txt.color = pointColor;
				}
		...
		for (int i = points.Count - 1; i >= 0; i--) {
			...
			//report_Event.getreport_Event.showv ();
		}
